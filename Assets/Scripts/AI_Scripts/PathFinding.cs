...
public class PathFinding : MonoBehaviour
{
    ...
    private void FindPath(int2 startPosition, int2 endPosition)
    {
        ...
        for (int x = 0; x < gridSize.x; x++){ //creates all nodes in grid
            for (int y = 0; y < gridSize.y; y++){
                PathNode pathNode = new PathNode();
                pathNode.x = x;
                pathNode.y = y;
        ...
        while(openList.Length > 0)
        {
            ...
            for (int i = 0; i < neighbourOffsetArray.Length; i++){
                ...
                //Validates neighbour of current Node
                if (!IsPositionInsideGrid(neighbourPosition, gridSize)){ //Cheks if is on grid
                    continue;
                }

                int neighbourNodeIndex = CalculateIndex(neighbourPosition.x, neighbourPosition.y, gridSize.x);

                if (closedList.Contains(neighbourNodeIndex)){ //Cheks if has already searched this node
                    continue;
                }

                PathNode neighbourNode = pathNodeArray[neighbourNodeIndex];
                if (!neighbourNode.isWalkable){ //Cheks if is Walkable
                    continue;
                }
        ...
        PathNode endNode = pathNodeArray[endNodeIndex];
        if(endNode.parentIndex == -1){ //Didn't find a path
            Debug.Log("Path not found!");
        }
        else{   //Path found
            NativeList<int2> path = CalculatePath(pathNodeArray, endNode);

            foreach(int2 pathPosition in path){ //Debug the path backwards
    ...
    /// <returns>Returns an empty list if couldn't reach the path, and a list that starts from the endNode to the starting position if find the path.</returns>
    private NativeList<int2> CalculatePath(NativeArray<PathNode> pathNodeArray, PathNode endNode){
        if (endNode.parentIndex == -1){ //path not found
            return new NativeList<int2>(Allocator.Temp);    //This is and empty List, you can also return null
        }
        else{   //path found, goes backwards picking the parents nodes
            NativeList<int2> path = new NativeList<int2>(Allocator.Temp);
            path.Add(new int2(endNode.x, endNode.y));
            PathNode currentNode = endNode;
            while (currentNode.parentIndex != -1){
